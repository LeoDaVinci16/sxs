...
def load_measure_points(excel_filename, magnitude_col):
    ...
    if magnitude_col not in df.columns:
        raise ValueError(f"Magnitude column '{magnitude_col}' not in Excel")
...
def main():
    ...
    df = load_measure_points(EXCEL_FILE, MAGNITUDE_COL)
