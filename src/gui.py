...
class SXS_GUI(tk.Tk):
    ...
    def ask_magnitude_column(self, columns, default=None):
        top = tk.Toplevel(self)
        top.title("Select Magnitude Columns")
        top.geometry("350x500")

        tk.Label(top, text="Select one or more columns:").pack()
        ...
        for col in columns:
            var = tk.BooleanVar()
            tk.Checkbutton(scroll_frame, text=col, variable=var).pack(anchor="w")
            vars_map[col] = var
    ...
    def run_batch_plots(self):
        ...
        magnitude_col = self.ask_magnitude_column(sample_df.columns)
